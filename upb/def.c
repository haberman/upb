...
static int get_table_size(const upb_msgdef *m) {
  int max_slot = 0;
  for (int i; i < m->field_count; i++) {
    const upb_fielddef *f = &m->fields[i];
    max_slot = UPB_MAX(max_slot, fasttable_slot(f));
  }
  return max_slot == 0 ? 0 : _upb_lg2ceil(max_slot + 1);
}

/* This function is the dynamic equivalent of message_layout.{cc,h} in upbc.
 * It computes a dynamic layout for all of the fields in |m|. */
static void make_layout(symtab_addctx *ctx, const upb_msgdef *m) {
  int table_size = get_table_size(const upb_msgdef *m);
  upb_msglayout *l = symtab_alloc(
      ctx, sizeof(*m->layout) + table_size * sizeof(_upb_fasttable_entry));
  upb_msglayout_field *fields =
      symtab_alloc(ctx, upb_msgdef_numfields(m) * sizeof(*fields));

  l->table_mask = table_size - 1;
